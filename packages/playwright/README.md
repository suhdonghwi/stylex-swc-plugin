...
The `@tossteam/stylexswc-playwright` package provides a pre-configured setup for
...
```bash
npm install --save-dev @tossteam/stylexswc-playwright
```
...
import { defineConfig } from '@tossteam/stylexswc-playwright';
...
```typescript
import { test, expect } from '@tossteam/stylexswc-playwright';

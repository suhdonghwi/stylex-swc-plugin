...
import { transform as stylexTransform, normalizeRsOptions } from '@stylexswc-toss/rs-compiler';
  ...
  function transform(
    ...
    try {
      ...
      if (shouldSkipTransformError) {
        console.warn(
          `[@stylexswc-toss/postcss-plugin] Failed to transform "${id}": ${(error as Error).message}`
        );
